...
#             nn.ConvTranspose1d(ngf*8, ngf*4, 4, 2, 1, 1, bias=False),
...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv1d(latent_size, ngf*8, 4, 2, 1, bias=False),
            #nn.ConvTranspose1d(latent_size, ngf*8, 4, 2, 1, bias=False),
            nn.BatchNorm1d(ngf*8),
            #nn.Dropout(0.5),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Upsample(scale_factor = 4, mode = 'nearest'),
            nn.Conv1d(ngf*8, ngf*4, 4, 2, 1, 1, bias=False),
            #nn.ConvTranspose1d(ngf*8, ngf*4, 4, 2, 1, 1, bias=False),
            nn.BatchNorm1d(ngf*4),
            #nn.Dropout(0.5),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Upsample(scale_factor = 4, mode = 'nearest'),
            nn.Conv1d(ngf*4, ngf*2, 4, 2, 2, bias=False),
            #nn.ConvTranspose1d(ngf*4, ngf*2, 4, 2, 1, bias=False),
            nn.BatchNorm1d(ngf*2),
            #nn.Dropout(0.5),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Upsample(scale_factor = 4, mode = 'nearest'),
            nn.Conv1d(ngf*2, ngf, 4, 2, 2, bias=False),
            #nn.ConvTranspose1d(ngf*2, ngf, 4, 2, 1, bias=False),
            nn.BatchNorm1d(ngf),
            #nn.Dropout(0.5),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Upsample(scale_factor = 4, mode = 'nearest'),
            nn.Conv1d(ngf, nc, 4, 2, 3, bias=False),
            #nn.ConvTranspose1d(ngf, nc, 4, 2, 1, bias=False),
            nn.Tanh()
        )
